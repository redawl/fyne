...
func TestWindow_Cursor(t *testing.T) {
	...
	runOnMain(func() {
		w.mouseMoved(10, float64(e.Position().Y+10))
	...
	*/
	runOnMain(func() {
		w.mouseMoved(10, float64(b.Position().Y+10))
...
func TestWindow_HoverableOnDragging(t *testing.T) {
	...
	runOnMain(func() {
		...
		w.mouseMoved(12, 12)
		w.mouseClicked(w.viewport, glfw.MouseButton1, glfw.Press, 0)
		w.mouseMoved(28, 12) // outside the 20x20 object
		w.mouseClicked(w.viewport, glfw.MouseButton1, glfw.Release, 0)
...
func TestWindow_DragEndWithoutTappedEvent(t *testing.T) {
	...
	runOnMain(func() {
		w.mouseMoved(11, 11)
		w.mouseClicked(w.viewport, glfw.MouseButton1, glfw.Press, 0)
		w.mouseMoved(10, 10) // Less than drag threshold
		w.mouseClicked(w.viewport, glfw.MouseButton1, glfw.Release, 0)
...
func TestWindow_MouseEventContainsModifierKeys(t *testing.T) {
	...
	runOnMain(func() { w.mouseMoved(7, 7) })
...
func (s *safeWindow) mouseMoved(xpos, ypos float64) {
	s.window.mouseMoved(s.viewport, xpos, ypos)
	s.processMouseMoved(s.newMousePosX, s.newMousePosY)
}

func (w *safeWindow) processResized(width, height int) {
	// Noop for tests
}
